...
router.post('/join', isNotLoggedIn, async (req, res, next) => {//회원가입 라우터
    ...
    try {
        ...
        //회원가입 할 때 hash화를 해서 회원가입시킴.(두번째 인자는 얼마나 복잡하게 할 것인지임, 숫자가 클수록 해킹 위험은 적지만 오래 걸림.)
        await User.create({
            email,
            nick,
...
//이게 실행되면 localStrategy.js을 찾아가서 그 파일을 실행한다!! (index.js에서 local()을 했기 때문인가? 어떻게 찾아간다는거지)
